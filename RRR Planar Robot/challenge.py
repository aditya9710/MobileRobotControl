...
class Robot:
    ...
    def forward(cls, theta_0: float, theta_1: float, theta_2: float) -> Tuple[float, float, float]:
        ...
        y = cls.link_1 * np.sin(theta_0) + cls.link_2 * np.sin(theta_0 + theta_1) + \
            cls.link_3 * np.cos(theta_0 + theta_1 + theta_2) - cls.y_position_error_term
...
class Visualizer:
    ...
    def __init__(self, world: World) -> None:
        ...
        pygame.display.set_caption('3R Planar Robot Manipulation')
...
def main() -> None:
    ...
    goal = (goal[0], goal[1], 30 * np.pi/180)
